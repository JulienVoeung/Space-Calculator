...
const { syncToServer, getCounterId, deleteCounter, syncFromServer } =
  useAPI(id);
...
<template lang="pug">
.column.justify-evenly(data-cy="countercomponent")
  ...
  .row.justify-center.items-end
    q-btn.q-ma-md.col-1(data-cy="increment" rounded, color="blue", @click="state.incr(id)", :disabled="state.isShared[id]")
    ...
    q-btn.q-ma-md.col-1(data-cy="decrement", rounded, color="blue", @click="state.decr(id)", :disabled="state.isShared[id]")
        ...
        q-dialog(v-model='alert')
          q-card
            ...
            q-card-section.q-pt-none
              ...
              q-field(data-cy="counterId", filled='' label='Counter ID' stack-label='' id="counterIdField")
                template(v-slot:control)
                  .self-center.full-width.no-outline(tabindex='0') {{ counterId(id) }}
              q-card-actions(align='right')
                q-btn(data-cy="shareOK" flat='' label='OK' color='primary' v-close-popup='')
